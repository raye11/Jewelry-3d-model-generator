...
def postprocessing_scripts(filter_out_extra_only=False, filter_out_main_ui_only=False):
    ...
    scripts_list = getattr(modules.scripts.scripts_postproc, "scripts", [])
    # 确保 scripts_list 不是 None
    if scripts_list is None:
        scripts_list = []

    return list(filter(
        lambda s: (not filter_out_extra_only or not getattr(s, "extra_only", False)) 
                  and (not filter_out_main_ui_only or not getattr(s, "main_ui_only", False)),
        scripts_list
    ))
