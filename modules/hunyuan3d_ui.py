...
sys.path.insert(0, os.path.join(hunyuan_dir, "hy3dgen"))
...
class Hunyuan3DProcessor:
    ...
    def load_models(self, is_mv_mode=False):
        # self.unload_model()
        ...
        try:
            ...
            pass
        if is_mv_mode:
            subfolder = "hunyuan3d-dit-v2-1-mv"
        else:
            subfolder = "hunyuan3d-dit-v2-1"

        model_full_path = os.path.join(self.model_path, subfolder)
        ...
        try:
            print(f"正在加载 Hunyuan3D 模型 ({'多视角' if is_mv_mode else '单视角'})...")
    ...
    def generate_3d(self, 
                    ...
                    num_chunks=8000):
        try:
            is_mv_mode = any(img is not None for img in [mv_image_front, mv_image_back, mv_image_left, mv_image_right])
            self.load_models(is_mv_mode=is_mv_mode)
            
            if is_mv_mode:
                # 多视角模式
                image_dict = {}
                for key, img in [("front", mv_image_front), ("back", mv_image_back),
                                ("left", mv_image_left), ("right", mv_image_right)]:
                    if img is not None:
                        if check_box_rembg and self.rmbg is not None:
                            if img.mode == 'RGBA':
                                img = self.rmbg(img.convert('RGB'))
                            elif img.mode == 'RGB':
                                img = self.rmbg(img)
                        image_dict[key] = img
                if not image_dict:
                    raise ValueError("多视角模式下至少需提供一张视图")
                input_image = image_dict
                main_preview = image_dict.get("front", next(iter(image_dict.values())))
            else:
                # 单图模式
                if image is None:
                    raise ValueError("单图模式下必须提供 image")
                if check_box_rembg and self.rmbg is not None:
                    if image.mode == 'RGBA':
