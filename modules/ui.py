...
The most important point: **SYMMETRY IS NON-NEGOTIABLE**. Every jewelry item must appear geometrically symmetrical from a single viewpoint to minimize 3D model hallucination.

1. **Geometric Integrity** (Highest Priority):
   - ALL components must have physically connected structures: rings require fully closed bands, chains must show interlocking links, pendants need visible attachment points to chains
   - Eliminate ANY floating elements, disconnected parts, or ambiguous thin strands (<2mm thickness)
   - Critical reinforcement: "thick structural supports", "continuous material flow", "zero overhangs"

2. **Camera Perspective** (Strict Enforcement):
   - Rings/bracelets: 35° elevation view showing inner band AND outer surface continuity
   - Pendants/brooches: Dead-center front view with PERFECT bilateral symmetry visible
   - Earrings: Left-side only, 45° 3/4 view exposing both front profile AND side attachment mechanism
   - Necklaces: Front view with chain path forming unbroken loop, pendant centered on midline

3. **Texture & Detail Protocol**:
   - Decorations MUST be volumetric: "raised relief patterns", "engraved channels", "carved textures" (never painted-on designs)
   - Maximum 2 distinct pattern types per item (e.g., ONLY "geometric borders + central sun motif")
   - Forbidden details: filigree, micro-engraving, woven textures, perforated patterns, or any feature under 3mm thickness

4. **Composition Non-Negotiables**:
   - Single object centered, occupying EXACTLY 50% of frame height
   - Pure white (#FFFFFF) background with ZERO shadows, color bleed, or gradients
   - NO human elements, NO hands, NO neck forms, NO display stands
   - NO text, logos, watermarks, borders, or design sketches

5. **Lighting Protocol**:
   - Medical-grade shadowless illumination: "even diffuse lighting from dome lightbox"
   - ZERO highlights, ZERO specular reflections, ZERO rim lighting
   - Critical reinforcement: "no surface variations visible from lighting"

6. **Scale & Structural Visibility**:
   - Human-scale proportions: rings at finger-width (18-22mm diameter), pendants at palm-size (40-60mm height)
   - Mandatory structural callouts: "clear hinge mechanism visible", "chain link interconnection exposed", "clasp structure fully apparent"
   - Zero perspective distortion: "orthographic projection style", "no fisheye effect"

### ETHNIC STYLE MASTERY (DYNAMIC ELEMENT SELECTION):
- **NEVER** use generic terms ("ethnic", "tribal", "ancient"). Specify EXACT cultural origin: "Qiandong Miao", "Liangshan Yi", "Amdo Tibetan"
- **DYNAMIC ELEMENT SELECTION**: For each request, randomly select 2-3 authentic elements from the CORRECT cultural database below. NEVER reuse the same element combination twice for the same culture:
  - **Miao/Hmong**: Butterfly Mother motif OR spiral horns OR silver coin fringe OR batik-inspired wave patterns (Qiandong style); bird totems OR thunder patterns (Rongshui style)
  - **Yi**: Coiled dragon heads OR rhombus clan symbols OR curved dagger motifs OR black-red ritual color blocking (Liangshan); star constellations OR horse hoof patterns (Chuxiong)
  - **Tibetan**: Dorje (thunderbolt) finials OR cloud collar borders OR Dzi bead patterns OR mantra engravings (Ü-Tsang); mountain spirit motifs OR turquoise coral clusters (Kham)
  - **Mongolian**: Horse head finials OR eternal knot patterns OR silver coin collars OR hairpin ribbons (Khalkha); wolf totem engravings OR sky god symbols (Buryat)
  - **Uyghur**: Pomegranate blossoms OR geometric Girih tiles OR jade mountain motifs OR vine scroll borders (Kashgar); almond-eyed motifs OR ceramic glaze patterns (Turpan)
  - **Zhuang**: Bronze drum sun stars OR bird totem clasps OR embroidered textile patterns OR silver horn shapes (Guangxi); frog spirit motifs OR rice field patterns (Yunnan)
- ALWAYS include material authenticity: "Miao silver coin elements" NOT "decorative circles", "Yi black lacquer inlay" NOT "dark patterns"

### NEGATIVE PROMPT REQUIREMENTS (MANDATORY FULL LIST):
deformed, blurry, low quality, text, watermark, signature, human, hand, model, mannequin, shadow, reflection, highlight, shiny, glossy, mirror, transparent, translucent, glass, crystal, diamond, sapphire, ruby, emerald, enamel, floating parts, disconnected elements, multiple objects, jewelry on person, wireframe, partial object, asymmetrical, thin strands, ultra-detailed, filigree, wirework, chainmail, mesh, threads, hairline cracks, close-up, extreme detail, fisheye, perspective distortion, distorted proportions, background, gradient, logo, branding, sketch, drawing, illustration, painting, 3D render, CGI, doll, toy, miniature, oversized, blurry connections, hidden joints, ambiguous attachments, organic shapes, fluid forms

### OUTPUT FORMAT:
Return ONLY valid JSON with two keys: "positive" and "negative". 
- Positive prompts: Start with "a high-quality photo of...", under 70 words, in English. MUST include: structural integrity callouts + exact cultural elements + lighting/background specs.
- Negative prompts: Copy the FULL mandatory list above without truncation.

Example for Tibetan Amulet:
{
    "positive": "a high-quality photo of an Amdo Tibetan ritual amulet box, Dorje thunderbolt finials with cloud collar borders, matte oxidized silver construction with visible hinge mechanism, centered on pure white background, orthographic projection, even shadowless studio lighting, human-scale proportions (50mm height), no text, no branding",
    "negative":  "deformed, blurry, low quality, text, watermark, signature, human, hand, model, mannequin, shadow, reflection, highlight, shiny, glossy, mirror, transparent, floating parts, disconnected elements, multiple objects, jewelry on person, wireframe, partial object, asymmetrical, thin strands, ultra-detailed, filigree, wirework, chainmail, mesh, threads, hairline cracks, close-up, extreme detail, fisheye, perspective distortion,  distorted proportions, background, gradient, logo, branding, sketch, drawing, illustration, painting, 3D render, CGI, doll, toy, miniature, oversized, blurry connections, hidden joints, ambiguous attachments, organic shapes, fluid forms"
}

Now generate the JSON based on the user's request. REMEMBER: 
1) SYMMETRY AND STRUCTURAL CLARITY OVERRIDE ALL ARTISTIC CONSIDERATIONS
2) For ethnic jewelry, DYNAMICALLY SELECT 2-3 AUTHENTIC ELEMENTS from the correct cultural database
3) NEGATIVE PROMPT MUST CONTAIN THE FULL MANDATORY LIST WITHOUT ABBREVIATION
"""
...
def create_ui():
    ...
    # === Hunyuan3D 共享组件 ===
    with gr.Tabs() as h_input_tabs:
        with gr.Tab("单图模式"):
            h_image = gr.Image(type="pil", label="输入图片（单视角）")
            h_image_mode = gr.State("single")
        with gr.Tab("多视角模式"):
            gr.Markdown("请至少上传一个视角（推荐：前 + 后 或 前 + 左 + 右）")
            with gr.Row():
                h_mv_front = gr.Image(type="pil", label="正面", height=200)
                h_mv_back = gr.Image(type="pil", label="背面", height=200)
            with gr.Row():
                h_mv_left = gr.Image(type="pil", label="左侧面", height=200)
                h_mv_right = gr.Image(type="pil", label="右侧面", height=200)
            h_image_mode = gr.State("multi")
    ...
    def hunyuan_generate_wrapper(
        mode,
        image_single,
        mv_front, mv_back, mv_left, mv_right,
        steps, guidance, seed, octree, rembg, chunks
    ):
        if mode == "single":
            return hunyuan_processor.generate_3d(
                image=image_single,
                steps=steps, guidance_scale=guidance, seed=seed,
                octree_resolution=octree, check_box_rembg=rembg, num_chunks=chunks
            )
        else:
            return hunyuan_processor.generate_3d(
                mv_image_front=mv_front, mv_image_back=mv_back,
                mv_image_left=mv_left, mv_image_right=mv_right,
                steps=steps, guidance_scale=guidance, seed=seed,
                octree_resolution=octree, check_box_rembg=rembg, num_chunks=chunks
            )
    ...
    with gr.Blocks(analytics_enabled=False) as hunyuan_interface:
        ...
        with gr.Tabs() as h_mode_tabs:
            # ========== 单图模式 Tab ==========
            with gr.Tab("单图模式", id="single"):
                h_image_single = gr.Image(type="pil", label="输入图片（单视角）")
            with gr.Tab("多视角模式", id="multi"):
                gr.Markdown("请至少上传一个视角（推荐：前 + 后）")
                with gr.Row():
                    h_mv_front = gr.Image(type="pil", label="正面", height=200)
                    h_mv_back = gr.Image(type="pil", label="背面", height=200)
                with gr.Row():
        ...
        def _on_tab_select(evt: gr.SelectData):
            return evt.value  # 返回 Tab 的 id

        h_mode = gr.State("single")
        h_mode_tabs.select(_on_tab_select, None, h_mode)

        # 共享参数
        with gr.Accordion("⚙️ 推理参数", open=False):
            h_seed = gr.Number(value=-1, label="随机种子(-1表示随机)")
            h_steps = gr.Slider(10, 100, value=40, step=1, label="推理步数")
            h_guidance = gr.Slider(1, 15, value=7.5, step=0.1, label="引导尺度")
            h_octree = gr.Slider(64, 512, value=384, step=8, label="分辨率")
        ...
        h_btn = gr.Button("生成模型", variant="primary")
        ...
        h_btn.click(
            fn=wrap_gradio_gpu_call(hunyuan_generate_wrapper),
            inputs=[
                h_mode,
                h_image_single,
                h_mv_front, h_mv_back, h_mv_left, h_mv_right,
                h_steps, h_guidance, h_seed, h_octree, h_rembg, h_chunks
            ],
